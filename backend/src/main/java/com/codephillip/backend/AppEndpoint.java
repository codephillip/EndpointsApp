 ...
 */
@Api(name = "endpoint", version = "v1", namespace = @ApiNamespace(ownerDomain = "backend.codephillip.com", ownerName = "backend.codephillip.com", packagePath=""))
public class AppEndpoint {

    // Make sure to add this endpoint to your web.xml file if this is a web application.

    private static final Logger LOG = Logger.getLogger(AppEndpoint.class.getName());
    ...
    public void removeTeacher(@Named("id") Long id) throws NotFoundException {
        Teacher record = findTeacherRecord(id);
        if(record == null) {
            throw new NotFoundException("Teacher Record does not exist");
        }
        ofy().delete().entity(record).now();
    ...
    private Teacher findTeacherRecord(Long id) {
